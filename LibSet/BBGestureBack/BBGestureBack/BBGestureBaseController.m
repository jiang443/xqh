...
-(void)bb_basePopViewController:(UIViewController *)viewController PopType:(BBPopType)popType{
    ...
    gestureBaseView.maskView.backgroundColor = [UIColor colorWithHue:0 saturation:0 brightness:0 alpha:BBMaskingAlpha];
    gestureBaseView.imgView.transform = CGAffineTransformMakeScale(BBWindowToScale, BBWindowToScale);
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [[UIApplication sharedApplication].delegate.window.rootViewController.view addObserver:self forKeyPath:@"transform" options:NSKeyValueObservingOptionNew context:bbListenTabbarViewMove];
        self.tag = 502001;
    }
    return self;
}
...
- (void)dealloc{
    [[UIApplication sharedApplication].delegate.window.rootViewController.view removeObserver:self forKeyPath:@"transform" context:bbListenTabbarViewMove];
}
