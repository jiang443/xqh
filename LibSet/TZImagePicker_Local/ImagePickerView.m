...
@interface ImagePickerView() <TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate> {
    //NSMutableArray *_selectedAssets; //已移动到头文件中提供对外访问  edited by jiang
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        _imagePickerVc.navigationBar.barTintColor = self.parentVc.navigationController.navigationBar.barTintColor;
        _imagePickerVc.navigationBar.tintColor = self.parentVc.navigationController.navigationBar.tintColor;
...
//    });
    ...
    _margin = 4;
    _itemWH = (self.tz_width - _margin - 4) / _colNumberToEdit - _margin;
    _layout.itemSize = CGSizeMake(_itemWH, _itemWH);
    _layout.minimumInteritemSpacing = _margin;
    _layout.minimumLineSpacing = _margin;
    [self.collectionView setCollectionViewLayout:_layout];
    self.collectionView.frame = CGRectMake(0, contentSizeH, self.tz_width, self.tz_height - contentSizeH);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _selectedPhotos.count) {
        ...
        NSURL *url = [bundle URLForResource:@"TZImages" withExtension:@"bundle"];
        bundle = [NSBundle bundleWithURL:url];
        cell.imageView.image = [UIImage imageNamed:@"AlbumAddBtn" inBundle:bundle compatibleWithTraitCollection:nil];
    ...
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] tz_containsString:@"GIF"] && self.allowPickingGif && !self.allowPickingMuitlpleVideo) {
            ...
            [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
                ...
                self->_collectionView.contentSize = CGSizeMake(0, ((self->_selectedPhotos.count + 2) / self->_colNumberToEdit ) * (self->_margin + self->_itemWH));
            }];
            [_parentVc presentViewController:imagePickerVc animated:YES completion:nil];
        }
    }
}
...
#pragma mark - 五类个性化设置，这些参数都可以不传，此时会走默认设置
    ...
    // 设置横屏下的裁剪尺寸
    // imagePickerVc.cropRectLandscape = CGRectMake((self.tz_height - widthHeight) / 2, left, widthHeight, widthHeight);
    /*
...
- (void)pushImagePickerController {
    ...
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
        ...
        [_parentVc presentViewController:_imagePickerVc animated:YES completion:nil];
    } else {
                ...
                [[TZImageManager manager] getCameraRollAlbum:NO allowPickingImage:YES completion:^(TZAlbumModel *model) {
                    [[TZImageManager manager] getAssetsFromFetchResult:model.result allowPickingVideo:NO allowPickingImage:YES completion:^(NSArray<TZAssetModel *> *models) {
                        ...
                        if (self.allowCrop) { // 允许裁剪,去裁剪
                            ...
                            [self->_parentVc presentViewController:imagePicker animated:YES completion:nil];
                        } else {
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    ...
    [_collectionView reloadData];
    // _collectionView.contentSize = CGSizeMake(0, ((_selectedPhotos.count + 2) / _colNumberToEdit ) * (_margin + _itemWH));
    
    dispatch_time_t time=dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC);
    dispatch_after(time, dispatch_get_main_queue(), ^{
        if (self->_delegate && [self->_delegate respondsToSelector:@selector(imageDidAdd)]){
            [self->_delegate imageDidAdd];
        }
    });
...
- (void)deleteBtnClik:(UIButton *)sender {
    ...
    [_collectionView performBatchUpdates:^{
        ...
        [self->_collectionView reloadData];
        if (self->_delegate && [self->_delegate respondsToSelector:@selector(imageDidDelete)]){
            [self->_delegate imageDidDelete];
        }
    }];
}


- (void) setShowTakePhotoBtn:(BOOL)value{
    if (value){
        _showSheet = NO;
        _allowPickingImage = YES;
    }
...
- (void) setShowSheetClick:(BOOL)value {
    if (value) {
...
- (void) setAllowPickingOriginPhotoClick:(BOOL)value {
    if (value) {
        _allowPickingImage  = YES;
...
- (void) setAllowPickingImageClick:(BOOL)value {
    if (!value) {
...
- (void) setAllowPickingGifClick:(BOOL)value {
    if (value) {
        _allowPickingImage  = YES;
...
- (void) setAllowPickingVideoClick:(BOOL)value {
    if (!value) {
        _allowPickingImage  = YES;
        if (!self.allowPickingGif) {
            self.allowPickingMuitlpleVideo  = NO;
...
- (void) setAllowCropClick:(BOOL)allowCropClick {
    if (allowCropClick) {
        self.maxCount = 1;
...
- (void) setNeedCircleCropClick:(BOOL)value {
    if (value) {
        self.allowCrop  = YES;
        self.maxCount = 1;
        self.allowPickingOriginalPhoto  = NO;
...
- (void) setAllowPickingMultipleVideoClick:(BOOL)value {
