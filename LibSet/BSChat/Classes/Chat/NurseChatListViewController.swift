//
//版权所属：jiang
//文件名称：ContactChatListViewController.swift
//代码描述：***
//编程记录：
//[创建][2018/6/3][jiang]:新增文件

import UIKit
import BSCommon
import SwiftEventBus

class NurseChatListViewController: BaseChatListViewController{
    let completeDataCell = ChatListCell()
    let nursingPlanCell = ChatListCell()
    ...
    override func getHeaderView() -> UIView{
        ...
        let rect = CGRect(x: 0, y: 0, width: self.view.frame.width, height: cellHeight*3)
        let view = UIView()
        view.frame = rect
        ...
        nursingPlanCell.frame = CGRect(x: 0, y: cellHeight, width: rect.width, height: cellHeight)
        nursingPlanCell.avatarImageView.image = ImageCenter.bundleImage("chat_nursing_plan", with: self.classForCoder)
        nursingPlanCell.avatarImageView.backgroundColor = UIColor.color(hex: "#FA694C")
        nursingPlanCell.nameLabel.text = "院外照护方案"
        nursingPlanCell.messageLabel.text = "暂无未完成任务"
        nursingPlanCell.timeLabel.text = ""
        nursingPlanCell.badgeView.isHidden = true
        view.addSubview(nursingPlanCell)
        nursingPlanCell.addLine()
        ...
        let gesture3 = UITapGestureRecognizer(target: self, action: #selector(goNursingPlan))
        gesture3.numberOfTapsRequired = 1
        nursingPlanCell.isUserInteractionEnabled = true
        nursingPlanCell.addGestureRecognizer(gesture3)
    ...
    @objc func goNursingPlan(){
        SwiftEventBus.post(Event.Message.nursingPlan.rawValue)
    }
    ...
    func updateNursingPlanBadge(_ badge:Int, time:String) {
        let cell = self.nursingPlanCell
        cell.badgeView.badgeValue = badge.getBadge()
        if badge > 0 {
            cell.badgeView.isHidden = false
            cell.messageLabel.text = "\(badge)个未完成任务"
