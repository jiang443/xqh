...
public class ThreadUtils {
    ...
    public static func threadOnMain(_ block:@escaping (() -> Void)){
        DispatchQueue.main.async(execute: block)
    }
    
    public static func threadOnAsync(_ block:@escaping (() -> Void)){
        Async.async(execute: block)
    }
    
    public static func threadOnDBQueue(_ block:@escaping (() -> Void)){
        AsyncDBQueue.async(execute: block)
    }
    
    //毫秒
    public static func threadOnAsyncAfter(_ time:UInt64,block:@escaping (() -> Void)){
        Async.asyncAfter(deadline: DispatchTime.now() + Double((Int64)(time * NSEC_PER_MSEC)) / Double(NSEC_PER_SEC), execute: block)
    }
    
    public static func threadOnHttpQueue(_ block:@escaping (() -> Void)){
        AsyncHttpQueue.async(execute: block)
    }
    
    public static func threadOnUploadImgQueue(_ block:@escaping (() -> Void)){
